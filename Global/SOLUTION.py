...
import numpy as np
import Global.NondominatedSorting as NS
# import NondominatedSorting as NS
...
class SOLUTIONSET:
    ...
    def best(self):
        ...
        Feasible = np.any(self.cons() <= 0, axis=1)
        Objs = self.objs()
        Decs, Cons, AddPropers = self.decs(), self.cons(), self.adds()
        # if np.all(Feasible, where=False):
        #     Best = []
        if self.problem.M > 1:
            temp = NS.NDSort(Objs[Feasible, :], Objs[Feasible, :].shape[0])
